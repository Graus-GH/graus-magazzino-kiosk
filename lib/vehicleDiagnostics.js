...
async function getDiagnosticIds() {
  ...
  // Geotab's full Diagnostic list is huge (thousands of generic OBD/J1939
  // entries that apply to no vehicle in particular) — without an explicit
  // limit the API's default page can come back without the handful we
  // actually care about, which looked identical to a naming mismatch.
  const diagnostics = await geotabCall("Get", { typeName: "Diagnostic", search: {}, resultsLimit: 50000 });
  const byKey = {};
  for (const key of Object.keys(DIAGNOSTIC_MATCHERS)) {
    ...
    if (!match) console.error("Diagnostic not found for", key, "— total diagnostics fetched:", diagnostics.length, "— sample names:", diagnostics.slice(0, 5).map(d => d.name).join(" | "));
  }
